...
//        view.addGestureRecognizer(tap)
    ...
    func update_database(reset_db:String){
        ...
        HttpRequestCenter().request_user_data_v2("update_database", send_dic: send_dic as Dictionary<String, AnyObject>) { (return_dic:Dictionary<String, AnyObject>?) in
            if return_dic != nil && !return_dic!.isEmpty{
                ...
                let alert = UIAlertController(title: "錯誤", message: "網路異常，更新失敗", preferredStyle: .alert)
                self.present(alert, animated: true, completion: nil)
            }
        }
    }
